...
## Corpus Check
- 35 files · ~57,308 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1188 nodes · 2411 edges · 84 communities (60 shown, 24 thin omitted)
- Extraction: 74% EXTRACTED · 26% INFERRED · 0% AMBIGUOUS · INFERRED: 628 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `dfcf0c0a`
...
Cohesion: 0.16
Nodes (37): Asset, CleanSlateOptimizer, InterceptorSystem, Drop-in replacement for NonLinearMIPOptimizer.      Interface contract:, EnhancedEngagementMatrix, MIPConfig, MIP 최적화 통합 설정 - 현실적 규모 버전, 교전 가능성 및 관련 파라미터 사전 계산 (최적화 #1) (+29 more)
...
Cohesion: 0.07
Nodes (26): _calculate_stress_metrics(), MultiMissileTracker, OptimizedAssignmentManager, 하이브리드 자료구조: 빠른 쓰기 O(1) + 빠른 탐색 O(1)~O(k)     - battery_to_threats: List[List[int, Trigger flash + immediately clean up engagement lines and threat dot., Called by MultiMissileTracker internals. Thread-safe via signal., 실시간 DWTA 분석 시뮬레이터 - GUI 지원 버전, Invalidate active threats count cache (+18 more)

### Community 7 - "Clean-Slate MIP Model Building"
Cohesion: 0.14
Nodes (16): int, str, apply_warmstart(), build_model(), DWTAProblem, extract_result(), Variable index mapping for HiGHS model., Pure function: DWTAProblem → (HiGHS model, VarMap). (+8 more)
...
Cohesion: 0.08
Nodes (12): DWTAMainWindow, Called on main thread when simulation thread exits., Fires every 250 ms on the main thread — refresh all visual elements., 누적 로그 방식: 신규 위협은 행 추가, 기존 위협은 상태만 갱신., Flash alert bar if there are unengaged high-danger threats., Insert a colour-coded log entry (always runs on main thread via signal)., Update solver time, warm-start, or objective value labels., Compatibility: tracker calls this after update_simulation(). (+4 more)

### Community 10 - "Scenario Generation & Deployment"
Cohesion: 0.07
Nodes (26): 1. 대응 관계 (ROS2 ↔ TA), 2. 전역 선언 (shared), 3.1 radar_node (10 Hz, 시간/월드 권위), 3.2 threat_assessment_node (5 Hz), 3.3 engageability_node (5 Hz, Pk 게이트), 3.4 planning_node (2 Hz, WTA), 3.5 launcher_node (이벤트 + 5 Hz)  — `Interceptor` 슬롯과 결합, 3.6 control_station_node (1 Hz) (+18 more)

### Community 11 - "Stress Metrics & Table Refresh"
Cohesion: 0.22
Nodes (14): bool, float, build_problem(), _check_feasibility(), _compute_distance(), _compute_k(), _compute_k_from_dist(), Clean Slate DWTA Optimizer — Log-Linear Formulation with HiGHS Direct API  수학적 동 (+6 more)
...
Cohesion: 0.07
Nodes (18): cli(), main(), _summary(), _Bus, _Clock, _Logger, now(), ok() (+10 more)

### Community 41 - "Community 41"
Cohesion: 0.05
Nodes (80): ControlStationNode, 통제소 노드 — 방어정책 발행 (전시/평시, 단발/연속, 위협당 최대 요격탄)., EngageabilityNode, OO 가능성 평가 노드 — 교전 가능성/명중률 매트릭스.  탄도탄 예상궤적(/tracks) + 요격체계 상태(/interceptor_status, DWTA ROS2 nodes (rclpy-compatible; runs under real ROS2 or the in-process shim)., LauncherNode, 발사대/요격체계 노드 — 교전계획 실행 + 요격탄 비행/판정.  교전계획(/engagement_plan)을 받아 요격탄을 발사하고, 비행중 요격, BallisticTrack (+72 more)
...
Cohesion: 0.21
Nodes (18): Assignment, Assignment, EngagementCell, (요격체계, 탄도탄) 교전 가능성 한 칸: 교전창 + 명중률., ThreatScore, GreedyWTA, LegacyGreedyAdapter, Pluggable Weapon-Target Assignment (WTA) backends for planning_node.  The PoC de (+10 more)
...
Cohesion: 0.14
Nodes (13): 1) ROS2 없이 (어디서나 — Windows/Linux/Mac, Python만 필요), 2) 실제 ROS2 (rclpy), code:block1 ([경보체계]              [통제체계 (결심)]                          [체계), code:bash (python3 ros2_dwta/run_poc.py 45      # 45초 시뮬레이션), code:bash (# 워크스페이스에 dwta_ros2/ 를 두고), dwta_ros2 — 실시간 DWTA C2 파이프라인 (ROS2 노드 모델), 다이어그램 ↔ 노드 매핑, 실시간/계층 스케줄링 메모 (조사 결과 반영) (+5 more)
...
## Knowledge Gaps
- **223 isolated node(s):** `str`, `bool`, `int`, `str`, `bool` (+218 more)
...
- **Why does `extract_result()` connect `Clean-Slate MIP Model Building` to `Stress Metrics & Table Refresh`, `Community 36`?**
  _High betweenness centrality (0.170) - this node is a cross-community bridge._
- **Why does `Node` connect `Community 41` to `Community 36`, `Community 70`?**
  _High betweenness centrality (0.090) - this node is a cross-community bridge._
- **Why does `CleanSlateOptimizer` connect `Clean-Slate Optimizer & Scenarios` to `Simulation Engine (MultiMissileTracker)`, `Clean-Slate MIP Model Building`, `Main Window GUI Layout`, `Stress Metrics & Table Refresh`, `Community 81`, `Community 82`?**
