...
## Corpus Check
- 33 files · ~55,325 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1120 nodes · 2201 edges · 86 communities (72 shown, 14 thin omitted)
- Extraction: 76% EXTRACTED · 24% INFERRED · 0% AMBIGUOUS · INFERRED: 520 edges (avg confidence: 0.52)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `875e8101`
...
Cohesion: 0.22
Nodes (23): Asset, CleanSlateOptimizer, InterceptorSystem, Drop-in replacement for NonLinearMIPOptimizer.      Interface contract:, Threat, EnhancedEngagementMatrix, MIPConfig, MIP 최적화 통합 설정 - 현실적 규모 버전 (+15 more)
...
Cohesion: 0.23
Nodes (7): OptimizationMetrics, Stress Test Metrics Collection System ====================================== MIP, Stress Test 전체 메트릭 수집 (연구 논문용 확장), StressTestMetrics, float, int, str
...
Cohesion: 0.21
Nodes (6): GreedyOptimizer, 몬테카를로 시뮬레이션에서 샘플링된 요격확률 설정, Greedy 알고리즘 실행 (공정한 비교를 위해 개선)                  개선 사항:         - 단순 First-fit이 아, 목적함수 계산: GA와 동일한 방식         MIN_DAMAGE: min Σ B_i * [Π (1 - x*k*P)]         = 기댓, K-factor 가져오기: 캐시에서 또는 거리 기반 계산, Greedy 알고리즘 기반 DWTA 최적화기
...
Cohesion: 0.24
Nodes (8): KFactorCache, K-factor 사전 계산 캐시 + 경량 실시간 계산 (최적화 #3), 모든 (threat, system) 조합의 k-factor 계산                  Args:             threats:, Asset, InterceptorSystem, Greedy DWTA Optimizer ===================== Greedy 알고리즘 기반 DWTA 최적화기 (독립 모듈)  전략, Greedy 모델 생성 - 데이터 구조 초기화, Threat
...
Cohesion: 0.29
Nodes (3): EnhancedEngagementMatrix, MIPConfig, Load realistic scenario with comprehensive threat configuration.

### Community 38 - "Community 38"
Cohesion: 0.16
Nodes (18): DefensePolicy, EngagementMatrix, EngagementPlan, InterceptorStatus, Message types for the DWTA ROS2 pipeline.  For the PoC these are plain dataclass, 발사대/요격체계 상태 (잔여 유도탄 수, 교전 상태)., PlanningNode, OO계획 수립/전송 노드 — WTA 교전계획.  위협 점수(/threat_scores) + 교전 매트릭스(/engagement_matrix) + (+10 more)

### Community 41 - "Community 41"
Cohesion: 0.22
Nodes (21): BallisticTrack, LaunchEvent, RadarStatus, 단일 탄도탄 추적 정보 (위치/속도/예상탄착/발사점)., TrackArray, RadarNode, 레이다/경보체계 노드 (sensor + world truth for the PoC).  10 Hz 루프로 탄도탄을 전진시키고 추적정보(Track, Asset (+13 more)
...
Cohesion: 0.13
Nodes (16): ControlStationNode, 통제소 노드 — 방어정책 발행 (전시/평시, 단발/연속, 위협당 최대 요격탄)., EngageabilityNode, OO 가능성 평가 노드 — 교전 가능성/명중률 매트릭스.  탄도탄 예상궤적(/tracks) + 요격체계 상태(/interceptor_status, DWTA ROS2 nodes (rclpy-compatible; runs under real ROS2 or the in-process shim)., build_nodes(), cli(), main() (+8 more)
...
Cohesion: 0.24
Nodes (13): Assignment, Assignment, EngagementCell, (요격체계, 탄도탄) 교전 가능성 한 칸: 교전창 + 명중률., ThreatScore, LegacyGreedyAdapter, Expected protected value: sum over threats of danger * (1 - Π(1-pk))., Seam for the existing greedy_optimizer.GreedyOptimizer.      Not used by default (+5 more)
...
Cohesion: 0.17
Nodes (11): 1) ROS2 없이 (어디서나 — Windows/Linux/Mac, Python만 필요), 2) 실제 ROS2 (rclpy), code:block1 ([경보체계]              [통제체계 (결심)]                          [체계), code:bash (python3 ros2_dwta/run_poc.py 45      # 45초 시뮬레이션), code:bash (# 워크스페이스에 dwta_ros2/ 를 두고), dwta_ros2 — 실시간 DWTA C2 파이프라인 (ROS2 노드 모델), 다이어그램 ↔ 노드 매핑, 실시간/계층 스케줄링 메모 (조사 결과 반영) (+3 more)
...
Cohesion: 0.24
Nodes (5): LauncherNode, 발사대/요격체계 노드 — 교전계획 실행.  교전계획(/engagement_plan)을 받아 유도탄을 발사하고(잔여탄 차감), 발사 이벤트 (/l, EngagementPlan, Battery, float
...
Cohesion: 0.33
Nodes (5): ThreatScores, OO평가 노드 — 위협 평가/점수화.  탄도탄 정보(/tracks) + 방어자산 정보(사전입력)를 받아 위협을 점수화하고 /threat_scor, ThreatAssessmentNode, Asset, TrackArray
...
## Knowledge Gaps
- **208 isolated node(s):** `str`, `bool`, `int`, `str`, `bool` (+203 more)
...
- **Why does `TacticalMapWidget` connect `GUI Controls & Event Handlers` to `Clean-Slate Optimizer & Scenarios`, `Community 33`, `Community 66`, `Tactical Map Widget (PyQtGraph)`, `Community 37`, `Main Window GUI Layout`, `Assignment Manager Data Structure`, `Community 61`?**
  _High betweenness centrality (0.080) - this node is a cross-community bridge._
- **Why does `Node` connect `Community 44` to `Community 36`, `Community 38`, `Community 70`, `Community 41`, `Community 53`, `Community 55`?**
